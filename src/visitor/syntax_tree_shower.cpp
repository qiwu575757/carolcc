...
void syntax_tree_shower::visit(tree_comp_unit &node) {
    g_index++;
    std::printf("here!\n");
    std::string this_node = "comp_unit_" + std::to_string(g_index);
    std::printf("%s\n", this_node.c_str());
    g.set("syntax_graph", graphType::ZhuangTaiTu, "ZhuangTaiTu", "describe attributes here");
    g.begin();
    g.ZhuangTaiTu_add(last_node, "comp_unit_" + std::to_string(g_index));
...
void syntax_tree_shower::visit(tree_func_def &node) {
    g_index++;
    std::string this_node = "func_def_" + std::to_string(g_index);
    std::printf("%s\n", this_node.c_str());
    g.ZhuangTaiTu_add(last_node, "func_def_" + std::to_string(g_index));
...
void syntax_tree_shower::visit(tree_block &node) {
    g_index++;
    std::string this_node = "block_" + std::to_string(g_index);
    std::printf("%s\n", this_node.c_str());
    g.ZhuangTaiTu_add(last_node, "block_" + std::to_string(g_index));
    last_node = this_node;
    if (node.block_item_list) node.block_item_list->accept(*this);
}
void syntax_tree_shower::visit(tree_const_decl &node) {
    g_index++;
    std::string this_node = "const_decl_" + std::to_string(g_index);
    std::printf("%s\n", this_node.c_str());
    g.ZhuangTaiTu_add(last_node, "const_decl_" + std::to_string(g_index));
    last_node = this_node;
...
void syntax_tree_shower::visit(tree_basic_type &node) {
    g_index++;
    std::string this_node = "basic_type_" + std::to_string(g_index);
    std::printf("%s\n", this_node.c_str());
    g.ZhuangTaiTu_add(last_node, "basic_type_" + std::to_string(g_index));
}
void syntax_tree_shower::visit(tree_const_def_list &node) {
    g_index++;
    std::string this_node = "const_def_" + std::to_string(g_index);
    std::printf("%s\n", this_node.c_str());
    g.ZhuangTaiTu_add(last_node, "const_def_" + std::to_string(g_index));
...
void syntax_tree_shower::visit(tree_const_init_val &node) {
    g_index++;
    std::string this_node = "const_init_val_" + std::to_string(g_index);
    std::printf("%s\n", this_node.c_str());
    g.ZhuangTaiTu_add(last_node, "const_init_val_" + std::to_string(g_index));
    last_node = this_node;
    std::printf("%s\n", this_node.c_str());
...
void syntax_tree_shower::visit(tree_const_exp &node) {
    g_index++;
    std::string this_node = "const_exp_" + std::to_string(g_index);
    std::printf("%s\n", this_node.c_str());
    g.ZhuangTaiTu_add(last_node, "const_exp_" + std::to_string(g_index));
    last_node = this_node;
    if (node.add_exp) node.add_exp->accept(*this);
}
void syntax_tree_shower::visit(tree_var_decl &node) {
    g_index++;
    std::string this_node = "var_decl_" + std::to_string(g_index);
    std::printf("%s\n", this_node.c_str());
    g.ZhuangTaiTu_add(last_node, "var_decl_" + std::to_string(g_index));
    last_node = this_node;
    if (node.b_type) node.b_type->accept(*this);
    last_node = this_node;
...
void syntax_tree_shower::visit(tree_exp &node) {
    g_index++;
    std::string this_node = "exp_" + std::to_string(g_index);
    std::printf("%s\n", this_node.c_str());
    g.ZhuangTaiTu_add(last_node, "exp_" + std::to_string(g_index));
    last_node = this_node;
    if (node.add_exp) node.add_exp->accept(*this);
}
void syntax_tree_shower::visit(tree_init_val &node) {
    g_index++;
    std::string this_node = "init_val_" + std::to_string(g_index);
    std::printf("%s\n", this_node.c_str());
    g.ZhuangTaiTu_add(last_node, "init_val_" + std::to_string(g_index));
    last_node = this_node;
...
void syntax_tree_shower::visit(tree_decl &node) {
    g_index++;
    std::string this_node = "decl_" + std::to_string(g_index);
    std::printf("%s\n", this_node.c_str());
    g.ZhuangTaiTu_add(last_node, "decl_" + std::to_string(g_index));
    last_node = this_node;
...
void syntax_tree_shower::visit(tree_var_def_list &node) {
    g_index++;
    std::string this_node = "var_def_list_" + std::to_string(g_index);
    std::printf("%s\n", this_node.c_str());
    g.ZhuangTaiTu_add(last_node, "var_def_list_" + std::to_string(g_index));
...
void syntax_tree_shower::visit(tree_var_def &node) {
    g_index++;
    std::string this_node = "var_def_" + std::to_string(g_index);
    std::printf("%s\n", this_node.c_str());
    g.ZhuangTaiTu_add(last_node, "var_def_" + std::to_string(g_index) + node.id);
    last_node = this_node;
...
void syntax_tree_shower::visit(tree_block_item_list &node) {
    g_index++;
    std::string this_node = "block_item_list_" + std::to_string(g_index);
    std::printf("%s\n", this_node.c_str());
    g.ZhuangTaiTu_add(last_node, "block_item_list_" + std::to_string(g_index));
...
void syntax_tree_shower::visit(tree_block_item &node) {
    g_index++;
    std::string this_node = "block_item_" + std::to_string(g_index);
    std::printf("%s\n", this_node.c_str());
    g.ZhuangTaiTu_add(last_node, "block_item_" + std::to_string(g_index));
    last_node = this_node;
...
void syntax_tree_shower::visit(tree_stmt &node) {
    g_index++;
    std::string this_node = "stmt_" + std::to_string(g_index);
    std::printf("%s\n", this_node.c_str());
    g.ZhuangTaiTu_add(last_node, "stmt_" + std::to_string(g_index));
    last_node = this_node;
...
void syntax_tree_shower::visit(tree_assign_stmt &node) {
    g_index++;
    std::string this_node = "assign_stmt_" + std::to_string(g_index);
    std::printf("%s\n", this_node.c_str());
    g.ZhuangTaiTu_add(last_node, "assign_stmt_" + std::to_string(g_index));
    last_node = this_node;
...
void syntax_tree_shower::visit(tree_return_stmt &node) {
    g_index++;
    std::string this_node = "return_stmt_" + std::to_string(g_index);
    std::printf("%s\n", this_node.c_str());
    g.ZhuangTaiTu_add(last_node, "return_stmt_" + std::to_string(g_index));
    last_node = this_node;
    if (node.exp) node.exp->accept(*this);
}
void syntax_tree_shower::visit(tree_return_null_stmt &node) {
    g_index++;
    std::string this_node = "return_null_stmt_" + std::to_string(g_index);
    std::printf("%s\n", this_node.c_str());
    g.ZhuangTaiTu_add(last_node, "return_null_stmt_" + std::to_string(g_index));
}
void syntax_tree_shower::visit(tree_l_val &node) {
    g_index++;
    std::string this_node = "l_val_" + std::to_string(g_index);
    std::printf("%s\n", this_node.c_str());
    g.ZhuangTaiTu_add(last_node, "l_val_" + std::to_string(g_index));
    last_node = this_node;
    if (node.array_ident) node.array_ident->accept(*this);
}
void syntax_tree_shower::visit(tree_number &node) {
    g_index++;
    std::string this_node = "number_" + std::to_string(g_index);
    std::printf("%s\n", this_node.c_str());
    g.ZhuangTaiTu_add(last_node, "number_" + std::to_string(g_index));
}
void syntax_tree_shower::visit(tree_primary_exp &node) {
    g_index++;
    std::string this_node = "primary_exp" + std::to_string(g_index);
    std::printf("%s\n", this_node.c_str());
    g.ZhuangTaiTu_add(last_node, "primary_exp" + std::to_string(g_index));
    last_node = this_node;
    if (node.exp) node.exp->accept(*this);
    last_node = this_node;
...
void syntax_tree_shower::visit(tree_unary_exp &node) {
    g_index++;
    std::string this_node = "unary_exp_" + std::to_string(g_index);
    std::printf("%s\n", this_node.c_str());
    g.ZhuangTaiTu_add(last_node, "unary_exp_" + std::to_string(g_index));
    last_node = this_node;
...
void syntax_tree_shower::visit(tree_mul_exp &node) {
    g_index++;
    std::string this_node = "mul_exp_" + std::to_string(g_index);
    std::printf("%s\n", this_node.c_str());
    g.ZhuangTaiTu_add(last_node, "mul_exp_" + std::to_string(g_index));
    last_node = this_node;
...
void syntax_tree_shower::visit(tree_add_exp &node) {
    g_index++;
    std::string this_node = "add_exp_" + std::to_string(g_index);
    std::printf("%s\n", this_node.c_str());
    g.ZhuangTaiTu_add(last_node, "add_exp_" + std::to_string(g_index));
    last_node = this_node;
    if (node.add_exp) node.add_exp->accept(*this);
    last_node = this_node;
    if (node.mul_exp) node.mul_exp->accept(*this);
...
void syntax_tree_shower::visit(tree_rel_exp &node) {
    g_index++;
    std::string this_node = "rel_exp_" + std::to_string(g_index);
    std::printf("%s\n", this_node.c_str());
    g.ZhuangTaiTu_add(last_node, "rel_exp_" + std::to_string(g_index));
    last_node = this_node;
    if (node.add_exp) node.add_exp->accept(*this);
    last_node = this_node;
...
void syntax_tree_shower::visit(tree_eq_exp &node) {
    g_index++;
    std::string this_node = "eq_exp_" + std::to_string(g_index);
    std::printf("%s\n", this_node.c_str());
    g.ZhuangTaiTu_add(last_node, "eq_exp_" + std::to_string(g_index));
    last_node = this_node;
...
void syntax_tree_shower::visit(tree_l_and_exp &node) {
    g_index++;
    std::string this_node = "l_and_exp_" + std::to_string(g_index);
    std::printf("%s\n", this_node.c_str());
    g.ZhuangTaiTu_add(last_node, "l_and_exp_" + std::to_string(g_index));
    last_node = this_node;
    if (node.eq_exp) node.eq_exp->accept(*this);
    last_node = this_node;
...
void syntax_tree_shower::visit(tree_l_or_exp &node) {
    g_index++;
    std::string this_node = "l_or_exp_" + std::to_string(g_index);
    std::printf("%s\n", this_node.c_str());
    g.ZhuangTaiTu_add(last_node, "l_or_exp_" + std::to_string(g_index));
    last_node = this_node;
    if (node.l_and_exp) node.l_and_exp->accept(*this);
    last_node = this_node;
...
void syntax_tree_shower::visit(tree_init_val_list &node){
    g_index++;
    std::string this_node = "init_val_list" + std::to_string(g_index);
    std::printf("%s\n", this_node.c_str());
    g.ZhuangTaiTu_add(last_node, "init_val_list" + std::to_string(g_index));
    last_node = this_node;
...
void syntax_tree_shower::visit(tree_func_fparams &node) {
    g_index++;
    std::string this_node = "func_fparams"+std::to_string(g_index);
    std::printf("%s\n",this_node.c_str());
    g.ZhuangTaiTu_add(last_node,"func_fparams"+std::to_string(g_index));
...
void syntax_tree_shower::visit(tree_func_fparam &node) {
    g_index++;
    std::string this_node = "func_fparam"+std::to_string(g_index);
    std::printf("%s\n",this_node.c_str());
    g.ZhuangTaiTu_add(last_node,"func_fparam"+std::to_string(g_index));
    last_node = this_node;
...
void syntax_tree_shower::visit(tree_func_fparamone &node) {
    g_index++;
    std::string this_node = "func_fparamone"+std::to_string(g_index);
    std::printf("%s\n",this_node.c_str());
    g.ZhuangTaiTu_add(last_node,"func_fparamone"+std::to_string(g_index));
    last_node = this_node;
    if(node.b_type)node.b_type->accept(*this);
    last_node = this_node;
}
void syntax_tree_shower::visit(tree_func_fparamarray &node) {
    g_index++;
    std::string this_node = "func_fparamarray"+std::to_string(g_index);
    std::printf("%s\n",this_node.c_str());
    g.ZhuangTaiTu_add(last_node,"func_fparamarray"+std::to_string(g_index));
    last_node = this_node;
    if(node.b_type)node.b_type->accept(*this);
    last_node = this_node;
    if(!node.exps.empty())
...
void syntax_tree_shower::visit(tree_const_val_list &node){
    g_index++;
    std::string this_node = "tree_const_val_list"+std::to_string(g_index);
    std::printf("%s\n",this_node.c_str());
...
void syntax_tree_shower::visit(tree_const_exp_list &node){
    g_index++;
    std::string this_node = "tree_const_exp_list"+std::to_string(g_index);
    std::printf("%s\n",this_node.c_str());
    g.ZhuangTaiTu_add(last_node,this_node);
    last_node = this_node;
...
void syntax_tree_shower::visit(tree_arrray_def &node){
    g_index++;
    std::string this_node = "tree_arrray_def"+std::to_string(g_index);
    std::printf("%s\n",this_node.c_str());
    g.ZhuangTaiTu_add(last_node,this_node);
    last_node = this_node;
...
void syntax_tree_shower::visit(tree_if_stmt &node){
    g_index++;
    std::string this_node = "tree_if_stmt"+std::to_string(g_index);
    std::printf("%s\n",this_node.c_str());
    g.ZhuangTaiTu_add(last_node,this_node);
    last_node = this_node;
...
void syntax_tree_shower::visit(tree_if_else_stmt &node){
    g_index++;
    std::string this_node = "tree_if_else_stmt"+std::to_string(g_index);
    std::printf("%s\n",this_node.c_str());
    g.ZhuangTaiTu_add(last_node,this_node);
    last_node = this_node;
    if(node.cond)node.cond->accept(*this);
...
void syntax_tree_shower::visit(tree_while_stmt &node){
    g_index++;
    std::string this_node = "tree_while_stmt"+std::to_string(g_index);
    std::printf("%s\n",this_node.c_str());
    g.ZhuangTaiTu_add(last_node,this_node);
    last_node = this_node;
    if(node.cond)node.cond->accept(*this);
...
void syntax_tree_shower::visit(tree_break_stmt &node){
    g_index++;
    std::string this_node = "tree_break_stmt"+std::to_string(g_index);
    std::printf("%s\n",this_node.c_str());
    g.ZhuangTaiTu_add(last_node,this_node);
    last_node = this_node;
}
void syntax_tree_shower::visit(tree_continue_stmt &node){
    g_index++;
    std::string this_node = "tree_continue_stmt"+std::to_string(g_index);
    std::printf("%s\n",this_node.c_str());
    g.ZhuangTaiTu_add(last_node,this_node);
    last_node = this_node;
}
void syntax_tree_shower::visit(tree_cond &node){
    g_index++;
    std::string this_node = "tree_cond"+std::to_string(g_index);
    std::printf("%s\n",this_node.c_str());
    g.ZhuangTaiTu_add(last_node,this_node);
    last_node = this_node;
    if(node.l_or_exp)node.l_or_exp->accept(*this);
...
void syntax_tree_shower::visit(tree_array_ident &node){
    g_index++;
    std::string this_node = "tree_array_ident"+std::to_string(g_index);
    std::printf("%s\n",this_node.c_str());
    g.ZhuangTaiTu_add(last_node,this_node);
    last_node = this_node;
...
void syntax_tree_shower::visit(tree_func_call &node){
    g_index++;
    std::string this_node = "tree_func_call"+std::to_string(g_index);
    std::printf("%s\n",this_node.c_str());
    g.ZhuangTaiTu_add(last_node,this_node);
    last_node = this_node;
...
void syntax_tree_shower::visit(tree_func_paramlist &node){
    g_index++;
    std::string this_node = "tree_func_paramlist"+std::to_string(g_index);
    std::printf("%s\n",this_node.c_str());
    g.ZhuangTaiTu_add(last_node,this_node);
    last_node = this_node;
